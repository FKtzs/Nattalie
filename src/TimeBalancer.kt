...
class TimeBalancer : Module() {
    ...
    fun onRender2D(event: Render2DEvent) {
        ...
        if (counterMode) {
            RenderUtils.drawShadow(startX, startY, 160, 21)
            ...
            when (setFrame.get().toLowerCase()) {
                "^" -> {
                    ...
                    GlStateManager.resetColor()
                    FontLoaders.T19.drawString("Timer：$m ReleaseTime：$s", (startX - 4 + 36).toFloat(), (startY + 7.5).toFloat(), Color(fontR.get(),fontG.get(),fontB.get(),fontA.get()).rgb)
                    GlStateManager.resetColor()
                }
                "<" -> {
                    RoundedUtil.drawRound(startX.toFloat(), startY.toFloat(), 3f, 22f, 3F, Color(frameR.get(), frameG.get(),frameB.get(), frameA.get()))
                    GlStateManager.resetColor()
                    FontLoaders.T19.drawString("Timer：$m ReleaseTime：$s", (startX - 4 + 36).toFloat(), (startY + 6.5).toFloat(), Color(fontR.get(),fontG.get(),fontB.get(),fontA.get()).rgb)
                    GlStateManager.resetColor()
                }
            }
            GlStateManager.resetColor()
